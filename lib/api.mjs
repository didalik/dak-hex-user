...
function deal (maker, kp, makeTxId, takerPK, balanceId) { // {{{1
  let { s, e, c, d } = this
  let claimants = [ 
    new Claimant(d.issuer.id, Claimant.predicateUnconditional())
  ]
  let amount = HEX_FEE
  let claimClaimableBalance = Operation.claimClaimableBalance({ balanceId })
  let manageData = Operation.manageData({ name: 'takerPK', value: takerPK })
  return makeClaimableBalance.call(this, claimants, maker, kp, amount,
    [claimClaimableBalance, manageData], Memo.hash(makeTxId)
  ).then(r => e.log('deal makerPK', maker.id, 'takerPK', takerPK, r));
}
...
function takeOffer (taker, kp, makerPK, makeTxId, amount, validity = '0') { // {{{1
  let { s, e, c, d } = this
  let claimants = [ 
