...
async function dealOffer ( // {{{1
  ...
  return signDeal.call(this, tx.toXDR(), 'dealOffer').then(txXdr => {
    ...
    return e.server.submitTransaction(tx).then(txR => e.log({
      tag: 'dealOffer',
      txId: txR.id,
    })).catch(e => {
...
export { // {{{1
  dealOffer, dealRequest, description, makeOffer, makeRequest,
