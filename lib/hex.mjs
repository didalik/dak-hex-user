...
import { 
  Asset, Claimant, Memo, Operation, xdr, 
  ...
  take (opts, streams, onmessage) { // {{{2
    ...
    let claimants = [ // createClaimableBalance {{{3
      new Claimant(
        ...
        : Claimant.predicateBeforeRelativeTime(opts.validity)
      ),
      new Claimant( // taker can reclaim anytime
        opts.taker.keypair.publicKey(),
        Claimant.predicateUnconditional()
      )
    ]
    let ccb = Operation.createClaimableBalance({ claimants,
    ...
    return new User(opts.taker).load().then(user => {
      taker = user
      return user.cb(ccb, Memo.hash(this.txId), opts.data).
        submit();
    }).then(txR => {
...
class Offer extends Make { // {{{1
  constructor (opts) { // {{{2
    super(opts)
    if (this.validity) {
      this.memo = Memo.text(`Offer ${this.validity}`)
    }
  }
...
class Request extends Make { // {{{1
  constructor (opts) { // {{{2
    super(opts)
    if (this.validity) {
      this.memo = Memo.text(`Request ${this.validity}`)
    }
  }
...
class User extends Account { // Stellar HEX User {{{1
  ...
  convertClawableHexaToHEXA (opts) { // opts: amount, memo | grantTxId {{{2
    opts.memo ??= Memo.hash(opts.grantTxId)
    let claimants = [
      new Claimant(
  ...
  grant (take, make) { // {{{2
    let ccb = Operation.claimClaimableBalance({
    ...
    return this.cb(ccb, Memo.hash(take.txId)).pay(this.network.hex.assets[0], amount, null, take.takerPK).submit().then(txR => {
      return {
        amount,
        takeTxId: take.txId,
        txId: txR.id,
    ...
    let claimants = [
      new Claimant(
        ...
        !or.validity || or.validity == '0' ? // seconds
          Claimant.predicateUnconditional()
        : Claimant.predicateBeforeRelativeTime(or.validity)
      ),
      new Claimant( // maker can reclaim anytime
        this.loaded.id,
        Claimant.predicateUnconditional()
      )
    ]
  ...
  repay (txId) { // {{{2
    ...
    delete this.transaction
    return this.cb(ccb, Memo.hash(txId)).submit().then(txR => ({
      balanceId: getClaimableBalanceId(txR.result_xdr),
      txId: txR.id,
    }));
  }
  ...
  take (or, opts) { // {{{2
    ...
    return this.cb(ccb, Memo.hash(or.txId), opts.data).
      submit().then(txR => {
        let take = {
          amount, 
...
function hexAssets (hex) { // {{{1
  hex.assets = [
    new Asset('ClawableHexa', hex.issuerClawableHexa),
    new Asset('HEXA', hex.issuerHEXA),
  ];
}
...
export { // {{{1
  ...
  offerDeleted, offerMade, parseHEXA,
}
